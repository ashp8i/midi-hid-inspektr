...
def convert_svg_to_png(svg_path, output_path, size):
    ...
    cairosvg.svg2png(
        url=svg_path, write_to=output_path, output_width=size, output_height=size
    )
...
def create_splash_screen(svg_path):
    ...
    cairosvg.svg2png(
        url=svg_path, write_to=logo_path, output_width=400, output_height=400
    )
    ...
    draw.rectangle([(radius, 0), (width - radius, height)], fill=(35, 35, 35, 230))
    draw.rectangle([(0, radius), (width, height - radius)], fill=(35, 35, 35, 230))
    ...
    draw.ellipse(
        (width - radius * 2, height - radius * 2, width, height), fill=(35, 35, 35, 230)
    )
            ...
            try:
                ...
                try:
                    title_font = ImageFont.truetype(
                        "DejaVuSans-Bold.ttf", title_font_size
                    )
                except:
    ...
    try:
        ...
        # Draw version
        draw.text(
            version_position, version, font=version_font, fill=(200, 200, 200, 255)
        )
    except Exception as e:
...
def main():
    ...
    convert_svg_to_png(
        svg_file, os.path.join("resources", "icons", "app_icon.png"), 512
    )
